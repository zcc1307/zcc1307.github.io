...
\begin{enumerate}
  \item Online gradient descent (OGD): $R(x) = \frac{1}{2\eta}\| x \|_2^2$, which is $\frac1\eta$-strongly convex wrt $\| \cdot \|_2$. FTRL with $R$ has regret
  ...
  \item $p$-norm algorithms ($p \geq 2$): It is known that $R(x) = \frac{1}{2\eta}\| x \|_p^2$ is $\frac1\eta$-strongly convex wrt $\| \cdot \|_p$. FTRL with $R$ has regret:
  \[ \Reg(T, x) \leq \frac{\| x \|_p^2}{2\eta} + \eta \sum_{t=1}^T \| g_t \|_q^2. \]
  If $\Ccal \subset \cbr{x: \| x\|_p \leq B_p}$, and for all $t$, $\| g_t \|_q \leq R_q$,
  setting $\eta = \frac{B_p}{R_q\sqrt{2T}}$ implies that
  \[ \Reg(T, \Ccal) \leq B_p R_q \sqrt{2T}. \]

  \item Exponentiated gradient (Hedge): consider the negative entropy regularizer
  ...
  (The above regularizer can also be used to deal with a scaled version of probability simplex: $\cbr{x: \forall i, x_i > 0, \sum_{i=1}^d x_i = B_1}$ for general $B_1 > 0$; we will skip the discussion for simplicity.)
...
\paragraph{Instantiations: Perceptron and Winnow.} We consider two settings of $R$'s:
\begin{enumerate}
